...
function template(response, temp){
      ...
      function sanitezeHTML (str){
        let temp = document.createElement('span');
        temp.textContent = str;
        return temp.innerHTML;
      };

        let HTMLchild = '<p><a href=\'cs/'+sanitezeHTML(item.id)+'\'>'+sanitezeHTML(item.id)+'</a>\
         - '+sanitezeHTML(item.denominazione_struttura) +" - "+ sanitezeHTML(item.comune) +" - "+ sanitezeHTML(item.link.url)+'</p>\
         <p>'+sanitezeHTML(item.descrizione_struttura)+'</p>';
   ...
   if(temp.filter == null) {
    resp.forEach(appendResponse);
      ...
      filtered = resp.filter( item => item.id == temp.filter);
      if(filtered.length==0){alert('Non risultano centri sportivi con questo id!')}
      filtered.forEach(appendResponse);
    }
